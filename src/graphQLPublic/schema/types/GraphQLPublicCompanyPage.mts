 ...
 * and none of those can be derived from a list of 24 rows. A client that has to fetch a second page
 ...
 *
 * `hasMore` is computed from the rows actually fetched, not from `total`, so it stays exact even
