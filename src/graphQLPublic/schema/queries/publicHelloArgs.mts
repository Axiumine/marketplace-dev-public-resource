...
export const publicHelloArgs = {
	...
	// ⚠️ **Nothing here writes `args.name` anywhere but the response.** This resolver used to
	// `console.debug` the argument, and it was the single planted marker that came back out of nine
	// service logs when the log audit measured them: an anonymous caller on the public surface chose what
	// got written to disk. Console output is also telemetry — it becomes `event.breadcrumbs` on any error
